...
function SidePlayer({ width, height, episodeLink, trailerValid }){
    // setTimeout(function(){
    ...
    useEffect(()=> {
        ...
        setWidth2(window.innerWidth / 2.2);
        // setHeight(iframeWidth / 1.7);

        window.addEventListener('resize', ()=> {
            if (window.innerWidth <= 653) {
                setWidth2(window.innerWidth / 2.5);
            } else {
                setWidth2(window.innerWidth / 2.2); 
            }
    ...
    return (
        ...
        <Box position="relative" display="inline-block" borderRadius={5} overflow="hidden" width="95vw" minWidth="200px" paddingRight="75px" marginleft="50px" height={iframeWidth2} maxHeight="80vh">
